 ...
 *				google-closure-compiler --js=esw.js --js_output_file=esw.min.js
 *
...
(function(baseEmbeddedSvc) {
	...
	/**
	 * Checks if we're currently in a Community or Site context.
	 *
	 * @returns {boolean} Is this a community or site context?
	 */
	...
	Snapins.prototype.getLightningOutParamsObj = function() {
		...
		// Note that if neither of these cases are true, LO would expect paramsObj to be undefined.
	...
	Snapins.prototype.bootstrapEmbeddedService = function(attributes) {
		if(typeof Promise !== "function") { // eslint-disable-line no-negated-condition
			this.loadScriptFromDirectory(
				...
				"promisepolyfill",
				function() {
					return this.bootstrapEmbeddedService(attributes);
				},
				true
			);
		} else {
	...
	Snapins.prototype.appendIFrame = function appendIFrame() {
		...
		window.addEventListener("beforeunload", function(e) {
			...
			// TODO: temporary flag until we can thoroughly test new approach, see W-6873184
	...
	Snapins.prototype.addSessionHandlers = function addSessionHandlers() {
		...
		this.addMessageHandler("session.updateMaster", function(data) {
			if(data) {
				...
				// TODO: move this back to liveagent.esw.js when we can have multiple handlers for message type, see CL 18184430.
	...
	Snapins.prototype.addMessageHandler = function addMessageHandler(name, handler) {
		if(this.messageHandlers[name]) {
			this.warning("Replacing an existing handler for message type " + name);
		}
