...
(function(baseEmbeddedSvc) {
	...
	Snapins.prototype.setupMessageListener = function setupMessageListener() {
		window.addEventListener("message", function(message) {
			...
			var isMessageOriginValidProtocol = embedded_svc.utils ? embedded_svc.utils.isProtocolHttpOrHttps(message.origin) : false;
			var iframeOrigin = embedded_svc.utils ? embedded_svc.utils.getHostnameFromUrl(this.settings.iframeURL) : undefined;
			var feature;
			var oldHost;
			var newHost;
			...
			if(payload && payload.method && embedded_svc.isMessageFromSalesforceDomain(messageOrigin) && isMessageOriginValidProtocol) {
				...
				if(payload.method === "session.onLoad" && messageOrigin === iframeOrigin && this.settings.iframeURL.indexOf(messageOrigin) === -1) {
