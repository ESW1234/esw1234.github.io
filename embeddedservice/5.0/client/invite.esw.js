 ...
 *				google-closure-compiler --js=invite.esw.js --js_output_file=invite.esw.min.js --rewrite_polyfills=false
 *
 ...
 */
	...
	var SNAPINS_INVITE_ID = "snapins_invite";
	...
	*/
	...
	/**
	* 228 Salesforce Platform Cookie Consent (ePrivacy) allows cookies to be set under the following conditions:
	* 1) the category of the cookie is allowed for the current user consent settings.
	* 2) the page is not loaded from a blocklist of optional cookies OR Experience Cloud site domains.
	*
	* Reach out "Regulatory Compliance" team for questions.
	*/
	InviteButtonTracker.prototype.hasCookieConsent = function(cookieName) {
		/** Condition 1 **/
		// See "eFAQ - EPrivacy Cookie" document for category definitions: https://salesforce.quip.com/XhWAAKGgxWal
		var cookieCategory = {
			"liveagent_invite_rejected_" : "Preferences"
		}
		var isCategoryAllowed = cookieCategory[cookieName] && embedded_svc.utils &&
			embedded_svc.utils.cookie && embedded_svc.utils.cookie.isCategoryAllowedForCurrentConsent ?
			embedded_svc.utils.cookie.isCategoryAllowedForCurrentConsent(cookieCategory[cookieName]) :
			false;

		/** Condition 2 **/
		var optionalCookies = ['liveagent_invite_rejected_'];
		// Give consent for all but some optional cookies when on certain domains.
		var noConsentDomains = [];
		var isCookieOrDomainAllowed = optionalCookies.indexOf(cookieName) < 0 ||
			noConsentDomains.indexOf(window.location.host) < 0;

		return isCategoryAllowed && isCookieOrDomainAllowed;
	};

	InviteButtonTracker.prototype.accept = function() {
		if(!this.hasInviteAfterAccept && this.hasCookieConsent("liveagent_invite_rejected_")) {
	...
	InviteButtonTracker.prototype.reject = function() {
		if(!this.hasInviteAfterReject && this.hasCookieConsent("liveagent_invite_rejected_")) {
			esw.setCookie("liveagent_invite_rejected_" + this.buttonId, true, false);
		}
		this.remove(true);
	};
	...
	InviteButton.prototype.setActive = function(active) {
		// Set whether snapins invite is "inert" based on if the invite is inactive.
		if(document.getElementById(SNAPINS_INVITE_ID)) {
			document.getElementById(SNAPINS_INVITE_ID).inert = !active;
		}
		this.active = active;
	};
	...
	InviteButton.prototype.acceptInvite = function() {
		...
		embedded_svc.utils.fireEvent("onInviteAccepted");
	};
	...
	* Called from the static html invite reject button
	...
	InviteButton.prototype.rejectInvite = function() {
		...
		embedded_svc.utils.fireEvent("onInviteRejected");
	};
	...
	function InviteAPI() {
		...
		if(document.getElementById(INVITE_AVATAR_DOM_ID)) {
			if(embedded_svc.settings.avatarImgURL) {
				...
				child = document.getElementById(INVITE_AVATAR_DOM_ID);
				// Using removeChild as IE doesn't support remove
