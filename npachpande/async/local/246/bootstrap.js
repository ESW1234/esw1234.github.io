...
(() => {
	...
	const MODAL_ISDISMISSED_CLASS = "isDismissed";
		...
		return new Promise((resolve, reject) => {
			try {
				...
				if(button && !button.classList.contains(CONVERSATION_BUTTON_LOADED_CLASS)) {
					...
					initializeConversationState().catch(error);
	...
	*/
	...
	function createConversationButton() {
		...
		buttonElement.setAttribute("dir", getHtmlTextDirection());
	...
	/**
     * Get the direction of the language currently used.
     * See https://sourcegraph.soma.salesforce.com/git.soma.salesforce.com/aura/aura/-/blob/aura-impl/src/main/java/org/auraframework/impl/context/LocalizationAdapterImpl.java?L227:19&popover=pinned
     */
	function getHtmlTextDirection() {
		switch (embeddedservice_bootstrap.settings.language) {
			case "ar": // Arabic
			case "fa": // Persian
			case "he": // Hebrew
			case "iw": // Hebrew
	...
	**********************************************************/
	...
	EmbeddedServiceBootstrap.prototype.createIframe = function createIframe() {
		return new Promise((resolve, reject) => {
			try {
				...
				// Set HTML direction based on language
				iframe.setAttribute("dir", getHtmlTextDirection());
	...
	/**
	 * Handles generating initial markup and simulating button click to bootstrap Embedded Messaging client.
	...
	EmbeddedServiceBootstrap.prototype.bootstrapEmbeddedMessaging = function bootstrapEmbeddedMessaging() {
		return new Promise((resolve, reject) => {
			try {
				if (!sessionExists()) {
					// TODO: [W-12654992] Handle auth mode
					embeddedservice_bootstrap.generateMarkup();

					handleClick().then(() => {
						resolve("[Bootstrap API] Created Embedded Messaging frame.");
					});
				} else {
					// No-op.
				}
			} catch(e) {
				reject("[Bootstrap API] Error bootstrapping Embedded Messaging: " + e);
			}
		});
	};
	...
	EmbeddedServiceBootstrap.prototype.maximizeIframe = function maximizeIframe(frame) {
		...
		if(frame) {
			if(!frame.classList.contains(MODAL_ISMAXIMIZED_CLASS)) {
				frame.classList.add(MODAL_ISMAXIMIZED_CLASS);
			}

			if(frame.classList.contains(MODAL_ISMINIMIZED_CLASS)) {
				frame.classList.remove(MODAL_ISMINIMIZED_CLASS);
			}

			if(frame.classList.contains(MODAL_ISDISMISSED_CLASS)) {
				frame.classList.remove(MODAL_ISDISMISSED_CLASS);
			}
	...
	EmbeddedServiceBootstrap.prototype.minimizeIframe = function minimizeIframe(frame, data) {
		...
		if(frame) {
			...
			if(isDismissed) {
				...
				if(!frame.classList.contains(MODAL_ISDISMISSED_CLASS)) {
					frame.classList.add(MODAL_ISDISMISSED_CLASS);
				}
			} else {
			...
			Promise.all([cssPromise, configPromise, getBusinessHoursInterval(), sessionDataPromise]).then(() => {
				...
				if ($A) {
					$A.eventService.addHandler({
						...
						includeFacets: true,
						handler: function () {
							alert("Received the force:logout event…");
							embeddedservice_bootstrap.userVerificationAPI.clearSession().then(() => {
								alert("Cleared chat session successfully");
							});
						}
					});
				}
