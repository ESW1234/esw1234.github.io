...
(function() {
	...
	/**
	 * Rejection function for Embedded Messaging initialization promise.
	...
	*/
	...
	function isChannelDisplayed(channel) {
		let isChannelDisplayed = (!isEmbeddedMessagingChannel(channel) && channel.isDisplayedOnPageLoad) ||
			(isEmbeddedMessagingChannel(channel) && channel.isVisible && channel.isDisplayedOnPageLoad)
	...
	******************************************************/
	...
	function finishInit() {
		...
		if (embeddedMessagingChannels.length > 0) {
			...
			// Add event listeners for Embedded Messaging channel.
	...
	******************************************************/
	...
	function validateAndReorder(channels) {
		// Create shallow copy of menuItems.
	...
	******************************************************/
	...
	/******************************************************
					Public runtime functions
						(Embedded Messaging)
	******************************************************/
	let visibilityChangeEventListener;
	let initializationFailureListener;

	/**
	 * Remove event listeners for Embedded Messaging state change events.
	 */
	function removeEmbeddedMessagingEventListeners() {
		window.removeEventListener("onEmbeddedMessagingChannelMenuVisibilityChanged", visibilityChangeEventListener);
		window.removeEventListener("onEmbeddedMessagingInitError", initializationFailureListener);
	}

	/**
	 * Add event listeners for Embedded Messaging state change events.
	 */
	function addEmbeddedMessagingEventListeners() {
		initializationFailureListener = function () {
			// Reject the Embedded Messaging initialization promise.
			...
			// Remove the Embedded Messaging option.
			removeEmbeddedMessagingMenuOption();
		}

		visibilityChangeEventListener = function (options) {
			if (options && options.detail && options.detail.devName) {
				...
				embedded_svc.menu.menuConfig.menuItems
					.filter(channel =>
						isEmbeddedMessagingChannel(channel) && channel.channel === options.detail.devName
					)
					.forEach(channel =>
		...
		const reorderAndReopenChannelMenu = function (formattedMenuItems, shouldReopenChannelMenu) {
			embedded_svc.menu.reorder(formattedMenuItems);
			if (shouldReopenChannelMenu) {
		...
		const addEmbeddedMessagingMenuOption = function () {
			...
			// Resolve the Embedded Messaging initialization promise if it hasn't been settled.
			...
			const shouldReopenChannelMenu = (menu && wasChannelMenuOpen);
			if (isReorderAllowed) {
				reorderAndReopenChannelMenu(formattedMenuItems, shouldReopenChannelMenu);
			} else {
				// Reorder API currently not available, add a timeout to invoke the API.
				setTimeout(() => {
					reorderAndReopenChannelMenu(formattedMenuItems, shouldReopenChannelMenu);
				}, REORDER_DELAY);
			}
		...
		const removeEmbeddedMessagingMenuOption = function() {
			...
			const shouldReopenChannelMenu = (formattedMenuItems.length > 1 && menu && wasChannelMenuOpen);
			if (isReorderAllowed) {
				reorderAndReopenChannelMenu(formattedMenuItems, shouldReopenChannelMenu);
			} else {
				// Reorder API currently not available, add a timeout to invoke the API.
	...
	embedded_svc.menu.initializeEmbeddedMessaging = function initializeEmbeddedMessaging(menuItemData) {
		...
		embedded_svc.utils.loadScriptFromUrl(
			"https://esw1234.github.io/godwin/assets/js/init-no-stc-override.js",
			function() {
