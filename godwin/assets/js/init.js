    ...
    (() => {
        ...
        AgentforceMessagingUserVerification.prototype.setIdentityToken = function setIdentityToken(identityTokenData) {
            try {
                ...
                const VALID_AUTHENTICATED_MODES = [AUTH_MODE.AUTH, AUTH_MODE.SALESFORCE_SESSION_AUTH];
                if (!VALID_AUTHENTICATED_MODES.includes(configUtils.getAuthMode())) {
         ...
         * For authenticated conversations, this method also hides the chat button.
         * For guest conversations, this method also emits the onEmbeddedMessagingButtonCreated event. 
         * For authenticated conversations, the onEmbeddedMessagingButtonCreated event is fired after setting the identity token.
         */
        function resetClientToInitialState() {
            if (isAuthenticatedMode() || embeddedservice_bootstrap.settings.hideChatButtonOnLoad) {
                if (hasEmbeddedMessagingButtonCreatedEventFired) {
        ...
        function handleFabReadyEvent(event) {
            ...
            if (conversationStatus !== CONVERSATION_STATUS.NOT_STARTED || (typeof embeddedservice_bootstrap.settings.hideChatButtonOnLoad === 'boolean' ? !embeddedservice_bootstrap.settings.hideChatButtonOnLoad : businessHoursUtils?.isWithinBusinessHours())) {
